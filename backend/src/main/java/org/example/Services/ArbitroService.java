...
public class ArbitroService
{
    ...
    public Arbitro atualizarArbitro ( Long id, ArbitroUpdateResquest request )
    {
        Arbitro arbitro = arbitroRepository.findById( id ).orElse( null );
        if ( arbitro != null )
        {
            arbitro.setNome( request.nome() );
            arbitro.setFederacao( request.federacao() );
            arbitro.setCategoria( request.categoria() );
            return arbitroRepository.save( arbitro );
        }
